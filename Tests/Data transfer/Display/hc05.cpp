...
namespace mcbelib {
    ...
    /**Read a char over the bus secure by reading the same char 
     * PACKAGES_READ times, then returning the most frequent char*/
    char hc05::readData() {
        if (active_connection) {
            char data[PACKAGES_READ];
            readString( data, PACKAGES_READ );

            int i, j, count, max = 0, max_index;
            char check;

            for (i=0; i<PACKAGES_READ; i++) {
                check = data[i];
                count = 0;

                for (j=0; j<PACKAGES_READ; j++) {
                    if (check == data[i]) {
                        count++;
                    }
                }
